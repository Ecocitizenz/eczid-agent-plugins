// ECZ-ID portable evidence review engine (dependency-free, Node 18+).
//
// A faithful port of the VS Code family engine (packages/family: detect.ts,
// valueLayer.ts) so a plugin running in any agent host reports the SAME
// evidence, the SAME deterministic Review Priority and the SAME contextual
// next actions as the extension, from the same product facts.
//
// Local-first: it lists file NAMES and PATHS under a root and never opens a
// file. It performs no network access. It never emits a score or a verdict.
