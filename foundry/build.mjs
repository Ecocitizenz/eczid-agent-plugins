...
function pluginReadme(def, ctx, bundle) {
  ...
  // Kiro Powers requires both of these in the README. They are emitted only once the public
  // pages exist, so the estate never ships a link that does not resolve.
  if (facts.compliance?.privacyPolicyUrl) L.push(`- Privacy policy: ${facts.compliance.privacyPolicyUrl}`);
  if (facts.compliance?.supportContact) L.push(`- Support contact: ${facts.compliance.supportContact}`);
  L.push("");
