...
The three read-only ECZ-ID Verifier tools below come from the MCP server `{{verifier.npm}}@{{verifier.version}}` (stdio). Hosts that run local stdio MCP servers get them from the `mcp.json` shipped alongside this skill; hosts that do not run local MCP servers will not have them.
...
- **Explain the vocabulary.** ResultStates: {{verifier.resultStates}}. Policy modes: {{verifier.policyModes}}. A ResultState is evidence about public proof, never a verdict about safety.
- **Explain what a result would and would not mean**, using the rules in "How to use the tools" above.
- **Point at the public read-only surfaces** the user can open themselves: the Resolver at {{urls.resolver}}, and the first-party machine descriptor at {{urls.machineDiscovery}} (public read-only routing metadata; GET and HEAD only).
- **Say how to get the tools**: install this plugin in a host that runs local stdio MCP servers, or run the same check in CI with the GitHub Action `{{verifier.action}}` ({{urls.githubActionDocs}}).
